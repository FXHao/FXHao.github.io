...
* 框架
  ...
  一个框架就好比一个公司，每一个部门都有特定的职能，部门与部门相互配合才能让公司运转起来，而框架则是模块与模块相互配合来完成工作的![](C:\Users\acer\Desktop/Django/框架.png)

#### **Django框架**

* 它遵循的是 MVT 的思想

  ![](C:\Users\acer\Desktop/Django/MVT模块.png)
...
* ​	在虚拟环境下
  ...
  可以先创建一个目录，在该目录下创建项目**test1**![](C:\Users\acer\Desktop/Django/项目目录.png)
...
* 一个项目由很多个应用组成的，每一个应用完成一个功能模块。这里示例一下图书---人物的管理维护
  ...
  ![](C:\Users\acer\Desktop/Django/应用目录.png)
...
* 这时就可以用 Pycharm 之类的编辑软件打开项目，来编辑里面的内容
  ...
  ![](C:\Users\acer\Desktop/Django/应用注册.png)
...
* Django数据库默认使用的是 **sqlite3** 数据库，若要更换数据库，需手动去更改下**settings.py**的配置
  ![](C:\Users\acer\Desktop/Django/数据库配置.png)
...
* 生成迁移文件
  ...
  ![](C:\Users\acer\Desktop/Django/生成迁移.png)
...
* 首先可以先把项目中（即`/test1`目录下）的`urls.py`进行配置![](C:\Users\acer\Desktop/Django/URL配置1.png)

> 这里配置只是正则并没有写具体，而是包含具体应用的 `urls.py` 文件，在应用的 `urls.py` 文件中写 url 和 视图函数 的对应关系。

* 然后创建应用（即在`/test1/booktest`目录下）的`urls.py` ，在进行配置![](C:\Users\acer\Desktop/Django/URL2.png)
...
* 说一下匹配过程
  ...
  ![](C:\Users\acer\Desktop/Django/匹配过程.png)
...
* 首先创建存放模板的文件夹
  ...
  ![](C:\Users\acer\Desktop/Django/模板.png)
...
* 同样在`settings.py`文件下进行配置
  ...
  ![](C:\Users\acer\Desktop/Django/模板路径.png)
...
* 代码和数据变量
  ...
  * 在模板中也可以使用变量，变量可能是从视图中传递过来的，也可能是在模板中定义的
    ...
    * 页面格式好像不支持，就截图吧~~
    ...
    ![](C:\Users\acer\Desktop\Django\变量传递.png)

  * 代码段，在模板文件中也可以使用代码，如一个for循环

    ![](C:\Users\acer\Desktop\Django\代码段.png)
...
* 修改`settings.py`文件
  ...
  ![](C:\Users\acer\Desktop/Django/本地化.png)
...
* 在应用下的`admin.py`中注册模型类![](C:\Users\acer\Desktop/Django/注册模型1.png)

* 在类模型下添加这个，这是返回给页面的

  ![](C:\Users\acer\Desktop/Django/注册模型.png)
